...
void ListLen(Gaslist*);                         //Длина списка
...
int Menu()
{
    ...
    puts("3 - Действия со списком АЗС");
...
int ListActions()
{
    ...
    puts("2 - Удалить элемент списка АЗС(НЕ ДОСТУПНО)");
    ...
    {
        scanf("%d", &selected);
        if(selected < 0 || selected > 3) puts("Данного пункта меню не существует");
